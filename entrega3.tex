...
demoró 953715.972 ms (15.9 min) y generó 438.672 arcos.
	...
	Ahora debemos crear el archivo de nodos, para ellos ocuparemos el formato ... el programa XXXX convierte el CSV creado en un archivo con ese formato.
	
	Investigar-----    
    
    Ahora debemos intercambiar los nombres por números. Para lo anterior ocuparemos la tabla
...
%	Para la corrección de bordes se utilizará el algoritmo $PageRank$
	...
	Para instalar postgres en Debian GNU/Linux se deben ejecutar la siguiente secuencia de comandos en la consola de comandos.
...
# (psql) y crea un usuario en el sistema de nombres 
# postgres.
...
# salir de la base de datos postgres
\q

# ingresar a la base de datos memoria
psql memoria
...
Los datos fueron extraidos de la base de datos del departamento de Ingenieria Civil Transporte de la FCFM mediante el comando \textit{pg\_dump} de postgres y almacenados en un archivo de texto.
...
# se cargan los datos con el cliente de postgres psql.
psql -d memoria -a -f /ruta/a/archivo/con/datos
